...
/* Sort array alphabetically action_font.js */
...
function percentLeft(a, b){
  var dLeft = b - a;
  return  Math.floor((a/dLeft) * 100);
}
...
/* check if item contains letters only action_font.js */
...
/* 
  Get rotation degress from transform 
  Returns angle e.g: 65
*/
function getRotationDegreesFromTransform(obj) {
...
/* 
  removes event listeners by cloning 
*/
...
/* 
  Removes from dom and removes event listeners 
*/
function removeFromDOM(el, node){
  var element = (node) ? node : document.getElementById(el),
      clone;
  if (element) {
    clone = removeAllEventListenersFromElement(element);
